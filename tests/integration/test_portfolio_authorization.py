...
def portfolio_case_payload(
    ...
    *,
    ...
    input_account = payload["input"].get("account")
    if visible_accounts and isinstance(input_account, dict):
        input_account["account_id"] = visible_accounts[0]
...
def test_downside_grid_requalification_must_precede_user_confirmation(
    migrated_settings: Settings,
) -> None:
    original = run_frozen_decision_case(
        migrated_settings,
        portfolio_case_payload(
            migrated_settings,
            "grid-requalification-time-original",
            portfolio_confirmation_command(unobligated_portfolio_proposal()),
        ),
        clock=GovernanceClock(),
    )
    assert original.report is not None
    proposal = relaxed_portfolio_proposal()
    action_policy_version_id = "synthetic-action-policy-grid-time-boundary"
    proposal["risk_budget"].update(
        action_policy_version_id=action_policy_version_id,
        downside_grid=["0.40", "0.90"],
    )
    ...
    command["confirmation"]["relaxation_evidence"] = relaxation_evidence(original.decision_event_id)
    requalification = downside_grid_requalification(
        original.decision_event_id,
        action_policy_version_id=action_policy_version_id,
    )
    requalification["locked_forward_confirmed_at"] = confirmed_at
    command["confirmation"]["downside_grid_requalification"] = requalification

    execution = run_frozen_decision_case(
        migrated_settings,
        portfolio_case_payload(
            migrated_settings,
            "grid-requalification-time-boundary",
            command,
        ),
        clock=GovernanceClock("2042-06-17T16:01:00Z"),
    )
...
def test_action_policy_version_cannot_bind_a_different_grid_in_disjoint_portfolio(
    migrated_settings: Settings,
) -> None:
    original = run_frozen_decision_case(
        migrated_settings,
        portfolio_case_payload(
            migrated_settings,
            "disjoint-action-policy-original",
            portfolio_confirmation_command(unobligated_portfolio_proposal()),
        ),
        clock=GovernanceClock(),
    )
    assert original.report is not None
    disjoint_proposal = reduced_portfolio_proposal()
    disjoint_proposal["portfolio_id"] = "synthetic-decision-portfolio-disjoint"
    disjoint_proposal["risk_budget"].update(
        action_policy_version_id="synthetic-action-policy-alpha",
        downside_grid=["0.40", "0.90"],
    )
    ...
    execution = run_frozen_decision_case(
        ...
        portfolio_case_payload(
            migrated_settings,
            "disjoint-action-policy-rebinding",
            portfolio_confirmation_command(disjoint_proposal),
        ),
        clock=GovernanceClock("2042-05-21T16:01:00Z"),
    )
...
def test_portfolio_case_rejects_contributing_model_account_outside_frozen_scope(
    migrated_settings: Settings,
) -> None:
    payload = portfolio_case_payload(
        migrated_settings,
        "out-of-scope-model-account",
        portfolio_confirmation_command(reduced_portfolio_proposal()),
    )
    payload["input"]["account"]["account_id"] = "synthetic-account-4017"

    with pytest.raises(ValueError, match="scoped case versions and account facts must agree"):
